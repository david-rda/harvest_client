<template>
    ...
    <div>
        ...
        <section class="mb-5" v-if="is_legal">
            ...
            <div class="app-single-button btn btn-primary" @click="addItem('founders')">დამფუძნებლის დამატება&nbsp;<BIconPlusLg /></div>
                    ...
                    <div class="form-item" v-for="item in form.inputs.items" :key="item.index" :data-index="Number(item.index)+1">
                        ...
                        <div class="row">
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].name.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="დასახელება" v-model="form.inputs.items[item.index].name">
                            </div>
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].model.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="მოდელი" v-model="form.inputs.items[item.index].model">
                            </div>
                        </div>
                        ...
                        <div class="row" v-if="engine">
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].hp.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="ძრავის სიმძლავრე (ცხ/ძ)" v-model="form.inputs.items[item.index].hp">
                            </div>
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].purpose.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="დანიშნულება" v-model="form.inputs.items[item.index].purpose">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].manufactured_in.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="ქვეყანა" v-model="form.inputs.items[item.index].manufactured_in">
                            </div>
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].manufactured_on.error.length">
                                ...
                                <input type="date" class="form-control" placeholder="თარიღი" v-model="form.inputs.items[item.index].manufactured_on">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].company_name.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="კომპანია" v-model="form.inputs.items[item.index].company_name">
                            </div>
                            <div class="form-input position-relative mt-3 col-lg-6 col-md-6 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].company_id.error.length">
                                ...
                                <input type="text" class="form-control" placeholder="ს/კ" v-model="form.inputs.items[item.index].company_id">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-input position-relative mt-3 col-lg-4 col-md-4 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].currency.error.length">
                                ...
                                <select class="form-select" v-model="form.inputs.items[item.index].currency" @change="calculate">
                            ...
                            <div class="form-input position-relative mt-3 col-lg-4 col-md-4 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].price.error.length">
                                ...
                                <input ref="price" type="text" class="form-control" placeholder="1 ერთეული ტექნიკის ღირებულება" @input="calculate" v-model="form.inputs.items[item.index].price">
                            </div>
                            <div class="form-input position-relative mt-3 col-lg-4 col-md-4 col-xs-12 col-sm-12" :data-error="!!form.state.items[item.index].quantity.error.length">
                                ...
                                <input ref="quantity" type="number" class="form-control" min="1" @input="calculate" v-model="form.inputs.items[item.index].quantity">
                            </div>
                        </div>

                        <div class="btn btn-danger mt-4" @click="removeItem('items',item.index)" :disabled="form.inputs.items.length==1">#{{ Number(item.index)+1 }} ტექნიკის წაშლა</div>
